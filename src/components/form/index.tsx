...
const index = ({ data }: { data: Bloc2_2 }) => {
  return (
    <div
      className="container mx-auto px-4 lg:px-[calc(32px)] mb-16"
      data-aos="fade-up"
    >
      <form className="form flex flex-col items-center gap-4 w-full">
        <div className="flex items-center gap-14 w-full">
          {data.btn_1.slice(0, 1).map((item, index) => (
            <label key={index} className="w-[100px]">
              {item}
            </label>
          ))}
          {data.btn_1.slice(1, 2).map((item, index) => (
            <input
              ...
              type="text"
              placeholder={item}
              className="w-full border border-secondary rounded-full px-4 py-3 focus:outline-none shadow-[-2px_0_16px_rgba(242,84,85,0.2)]"
            />
          ))}
        </div>
        <div className="flex items-center gap-14 w-full">
        ...
        <div className="flex items-center gap-14 w-full">
          ...
          <textarea className="w-full border border-secondary rounded-full py-3 focus:outline-none shadow-[-2px_0_16px_rgba(242,84,85,0.2)]"></textarea>
        </div>
        <div className="flex items-center gap-14 w-full">
          {data.btn_4.slice(0, 1).map((item, index) => (
            <label key={index} className="w-[90px]">
              {item}
            </label>
          ))}
          <div className="flex items-center space-x-2 flex-col lg:flex-row">
            ...
            {data.btn_4.slice(2, 3).map((item, index) => (
              <p key={index} className="text-[18px] text-[#999]">
                (*{item})
              </p>
            ))}
          </div>
